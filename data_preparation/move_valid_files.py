...
names = [name for name in os.listdir(data_path) if "DS_Store" not in name]
...
for i, name in tqdm(enumerate(valid_names)):
    ...
    copyfile(os.path.join(data_path, name, f'{name}_protein_processed2.pdb'), f'data/PDBBind_processed/{name}/{name}_protein_processed2.pdb')
